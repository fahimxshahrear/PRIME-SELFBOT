...
aayush72 = commands.Bot(description='__PrimeB0T__',
                           command_prefix='P',
                           self_bot=True,
                           intents=intents)
status_task = None

aayush72.remove_command('help')

aayush72.whitelisted_users = {}

aayush72.antiraid = False
        ...
        Center.XCenter(
            ...
            f"""
				...
				|__/     |__/      |__/|__/ |__/ |__/ \_______/
                                               					...
                                               					Discord - @therealaayush
                                               


[+] -------------------------------- ] | [ S E L F B O T - {aayush72.user.name}  ] | [ ------------------------------------ [+]
""",
                1,
            )
        )
...
async def on_message(message):
    ...
    await aayush72.process_commands(message)
...
async def on_member_ban(guild, user):
    if aayush72.antiraid is True:
        try:
            async for entry in guild.audit_logs(
                    limit=1, action=discord.AuditLogAction.ban):
                if (guild.id in aayush72.whitelisted_users.keys()
                        and entry.user.id
                        in aayush72.whitelisted_users[guild.id].keys()
                        and entry.user.id != aayush72.user.id):
...
async def antinuke(ctx, antiraidparameter=None):
    ...
    if str(antiraidparameter).lower() == 'true' or str(
        ...
        await ctx.send(
            f'- **[! ERROR] ** `USAGE : {aayush72.command_prefix}antiraid [true/false]`'
        )


#WWHITE CMD=======================================================================================================================================================================================================
#ADDTION WHITELIST
@aayush72.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send(
            f'[ERROR]: USAGE :  {aayush72.command_prefix}whitelist <user>')
    else:
        if ctx.guild.id not in aayush72.whitelisted_users.keys():
            aayush72.whitelisted_users[ctx.guild.id] = {}
        if user.id in aayush72.whitelisted_users[ctx.guild.id]:
            await ctx.send("- `" + user.name.replace("*", "\*").replace(
                "`", "\`").replace("_", "\_") + "#" + user.discriminator +
                           "`-` ** ALREADY WHITELISTED [!]**")
        else:
            aayush72.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("# __PrimeB0T__\n`-` **WHITELISTED**" + user.name.replace(
                "*", "\*").replace("`", "\`").replace("_", "\_") + "#" +
                           user.discriminator + "`")


#CHECK WHITELIST
@aayush72.command(aliases=['showwl'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '# __PrimeB0T__\n`-`**ALL WHITELISTED USERS:**`\n'
        for key in aayush72.whitelisted_users:
            for key2 in aayush72.whitelisted_users[key]:
                user = aayush72.get_user(key2)
                whitelist += f'• {user.mention} ({user.id}) IN {aayush72.get_guild(key).name}\n'
        await ctx.send(whitelist)
    else:
        whitelist = f'# __PrimeB0T__\n`-` **WHITELISTED USERS IN {ctx.guild.name}:**`\n'
        for key in aayush72.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in aayush72.whitelisted_users[ctx.guild.id]:
                    user = aayush72.get_user(key2)
...
@aayush72.command(aliases=['removewl'])
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in aayush72.whitelisted_users.keys():
            ...
            return
        if user.id in aayush72.whitelisted_users[ctx.guild.id]:
            aayush72.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = aayush72.get_user(user.id)
...
#WHITELIST CLEAR
@aayush72.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    aayush72.whitelisted_users.clear()
...
async def send_selfbotinfo_message(channel):
    await channel.send(
        f"# __PrimeB0T__\n`-` **VERSION** : `SELFBOT V5` \n`-` **LANGUAGE** : `PYTHON & JS`\n`-` **NO. OF COMMANDS** : `70+`\n`-` **ASKED BY** : `{aayush72.user.name}`\n`-` **CREATOR** : `@therealaayush`\n\n`THERE ARE BOTH PREFIX & NON PREFIX COMMANDS`"
    )
# SERVER INFO.
...
async def massdmfriends(ctx, *, message):
    for user in aayush72.user.friends:
        try:
            time.sleep(.1)
...
# NITRO GEN
@aayush72.command(aliases=["nitrogen"])
async def nitro(ctx):
    try:
        await ctx.message.delete()
        code = ''.join(
...
# STREAM CREATOR
@aayush72.command(aliases=["streaming"])
async def stream(ctx, *, message):
    stream = discord.Streaming(
        name=message,
        url="https://twitch.tv/https://Wallibear",
    )
    await aayush72.change_presence(activity=stream)
...
# PLAY CREATOR
@aayush72.command(aliases=["playing"])
async def play(ctx, *, message):
    game = discord.Game(name=message)
    await aayush72.change_presence(activity=game)
...
# WATCH CREATOR
@aayush72.command(aliases=["watch"])
async def watching(ctx, *, message):
    await aayush72.change_presence(activity=discord.Activity(
...
@aayush72.command(aliases=["listen"])
...
@aayush72.command(aliases=[
    "stopstreaming", "stopstatus", "stoplistening", "stopplaying",
    "stopwatching"
])
async def stopactivity(ctx):
    await ctx.message.delete()
    await aayush72.change_presence(activity=None, status=discord.Status.dnd)
...
# CONNECT VC
@aayush72.command(aliases=['247'])
@commands.check(is_authorized)
async def connectvc(ctx, channel_id):
    try:
        channel = aayush72.get_channel(int(channel_id))
...
async def copyserver(ctx, source_guild_id: int, target_guild_id: int):
    source_guild = aayush72.get_guild(source_guild_id)
    target_guild = aayush72.get_guild(target_guild_id)
...
async def change_hypesquad(ctx):
    ...
    try:
        msg = await aayush72.wait_for('message', check=check, timeout=60)
...
# AVATAR
@aayush72.command(aliases=['av','ava'])
@commands.check(is_authorized)
...
# PURGE CMD...
@aayush72.command(aliases=['purge, clear'])
@commands.check(is_authorized)
...
# NSFW
@aayush72.command(aliases=['fuck', 'fx', '18+', 'xxx', 'nsfw'])
async def waifu(ctx):
    try:
...
# SERVER BANNER
@aayush72.command(aliases=['serverbanner'])
...
# SERVER PFP
@aayush72.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
async def nukezzz(ctx):
    ...
    message_text = '# FUCKED BY 404UserNotFound :  https://discord.gg/RxcsEf83mY ||@everyone||'
...
@aayush72.command(aliases=['bal', 'ltcbal'])
async def getbal(ctx, ltcaddress):
    ...
    message += f"`-` **ASKED BY** : `{aayush72.user.name}`\n"
...
#MESSAGE UPLOADER
@aayush72.command(name='send')
async def send_message(ctx, *, message: str):
    try:
        
        for channel_id in channel_ids:
            channel = aayush72.get_channel(channel_id)
...
# STATUS ROTATOR
aayush72.load_extension("status_rotator")


token = config.get('Token')
aayush72.run(token, bot=False)
